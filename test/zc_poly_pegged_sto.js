...
import { setUpPolymathNetwork, deployGPMAndVerifyed, deployPOLYPeggedSTOAndVerified } from "./helpers/createInstances";

const POLYPeggedSTO = artifacts.require("./POLYPeggedSTO.sol");
const MockOracle = artifacts.require("./MockOracle.sol");
const SecurityToken = artifacts.require("./SecurityToken.sol");
const GeneralTransferManager = artifacts.require("./GeneralTransferManager");
const PolyTokenFaucet = artifacts.require("./PolyTokenFaucet.sol");
const STGetter = artifacts.require("./STGetter.sol");
const POLYPeggedSTOFactory = artifacts.require("./POLYPeggedSTOFactory.sol");
const POLYPeggedSTOProxy = artifacts.require("./POLYPeggedSTOProxy.sol");

const Web3 = require("web3");
let BN = Web3.utils.BN;
const web3 = new Web3(new Web3.providers.HttpProvider("http://localhost:8545")); // Hardcoded development port

contract("POLYPeggedSTO", async (accounts) => {
    let e18;
    let e16;
    ...
    let I_POLYPeggedSTOFactory;
    let I_POLYOracle;
    let I_STFactory;
    let I_SecurityToken;
    let I_STRProxied;
    let I_MRProxied;
    let I_POLYPeggedSTO_Array = [];
    let I_PolyToken;
    let I_PolymathRegistry;
    let P_POLYPeggedSTOFactory;
    let I_STRGetter;
    let I_STGetter;
    let stGetter;
    let I_ERC20Token;
...
/*    function configure(
    ...
    before(async () => {
        ...
        // STEP 3: Deploy the POLYPeggedSTOFactory
        [I_POLYPeggedSTOFactory] = await deployPOLYPeggedSTOAndVerified(POLYMATH, I_MRProxied, STOSetupCost);
        [P_POLYPeggedSTOFactory] = await deployPOLYPeggedSTOAndVerified(POLYMATH, I_MRProxied, new BN(web3.utils.toWei("500")));
        ...
        POLYPeggedSTOFactory:              ${I_POLYPeggedSTOFactory.address}
        POLYPeggedSTOFactory(Paid):        ${P_POLYPeggedSTOFactory.address}
        -----------------------------------------------------------------------------
        `);
    });
    ...
    describe("Test STO deployment", async () => {
        it("Should successfully attach the first STO module to the security token", async () => {
            ...
            assert.equal((await I_POLYPeggedSTO_Array[stoId].startTime.call()).toString(), _startTime[stoId].toString(), "Incorrect _startTime in config");
            ...
            assert.equal(
                (await I_POLYPeggedSTO_Array[stoId].nonAccreditedLimit.call()).toString(),
                _nonAccreditedLimit[stoId].toString(),
                "Incorrect _nonAccreditedLimit in config"
            );
            assert.equal(
                (await I_POLYPeggedSTO_Array[stoId].maxNonAccreditedInvestors.call()).toString(),
    ...
    describe("Test modifying configuration", async () => {
        ...
        it("Should not allow configuration -- fail Sender is not factory", async () => {
            let stoId = 0;
            await catchRevert(I_POLYPeggedSTO_Array[stoId].configure(
                _startTime[stoId],
                _endTime[stoId],
                _cap[stoId],
                _rate[stoId],
        ...
        it("Should successfully Modify Times before starting the STO -- fail because of bad owner", async () => {
            let stoId = 0;
            await catchRevert(
                I_POLYPeggedSTO_Array[stoId].modifyTimes(new BN(currentTime).add(new BN(duration.days(15))), new BN(currentTime).add(new BN(duration.days(55))), { from: POLYMATH })
            );
        });
        ...
        it("Should successfully Modify Times before starting the STO -- fail start time is in th past", async () => {
            let stoId = 0;
            await catchRevert(
                I_POLYPeggedSTO_Array[stoId].modifyTimes(new BN(currentTime).sub(new BN(duration.days(15))), new BN(currentTime).add(new BN(duration.days(55))), { from: ISSUER })
            );
        });
        ...
        it("Should successfully Modify Times before starting the STO -- fail end time is before start time", async () => {
            let stoId = 0;
            await catchRevert(
                I_POLYPeggedSTO_Array[stoId].modifyTimes(new BN(currentTime).sub(new BN(duration.days(15))), new BN(currentTime).add(new BN(duration.days(10))), { from: ISSUER })
            );
        });
        ...
        it("Should successfully Modify Times before starting the STO", async () => {
            ...
            await I_POLYPeggedSTO_Array[stoId].modifyTimes(_startTime, _endTime, { from: ISSUER });
            assert.equal((await I_POLYPeggedSTO_Array[stoId].startTime.call()).toString(), _startTime.toString(), "Incorrect _startTime in config");
            assert.equal((await I_POLYPeggedSTO_Array[stoId].endTime.call()).toString(), _endTime.toString(), "Incorrect _endTime in config");
            await revertToSnapshot(snapId);
        });
        ...
        it("Should Fail to Modify Times after starting the STO", async () => {
            ...
            await catchRevert(I_POLYPeggedSTO_Array[stoId].modifyTimes(new BN(tempTime1), new BN(tempTime2), { from: ISSUER }));
            await revertToSnapshot(snapId);
        });
        ...
        it("Should successfully Modify Cap before starting the STO -- fail because of bad owner", async () => {
            let stoId = 0;
            await catchRevert(I_POLYPeggedSTO_Array[stoId].modifyCap((new BN(250000000).mul(e18)), { from: POLYMATH }));
        });

        it("Should successfully Modify Cap before starting the STO -- fail because cap = 0", async () => {
            let stoId = 0;
            await catchRevert(I_POLYPeggedSTO_Array[stoId].modifyCap(new BN(0), { from: ISSUER }));
        });

        it("Should successfully Modify Cap before starting the STO", async () => {
            let stoId = 0;
            let snapId = await takeSnapshot();
            await I_POLYPeggedSTO_Array[stoId].modifyCap((new BN(250000000).mul(e18)), { from: ISSUER });

            assert.equal((await I_POLYPeggedSTO_Array[stoId].cap.call()).toString(), new BN(250000000).mul(e18).toString(), "Cap doesn't set as expected");
            await revertToSnapshot(snapId);
        });
        ...
        it("Should Fail to Modify Cap after starting the STO", async () => {
            ...
            await increaseTime(duration.days(3));
            assert.equal(await I_POLYPeggedSTO_Array[stoId].isOpen(), true, "STO is not Open");
            await catchRevert(I_POLYPeggedSTO_Array[stoId].modifyCap((new BN(300000000).mul(e18)), { from: ISSUER }));
            await revertToSnapshot(snapId);
        });
        ...
        it("Should successfully Modify Rate before starting the STO -- fail because of bad owner", async () => {
            let stoId = 0;
            await catchRevert(I_POLYPeggedSTO_Array[stoId].modifyRate(new BN(1).mul(e16), { from: POLYMATH }));
        });

        it("Should successfully Modify Rate before starting the STO -- fail because rate = 0", async () => {
            let stoId = 0;
            await catchRevert(I_POLYPeggedSTO_Array[stoId].modifyRate(new BN(0), { from: ISSUER }));
        });

        it("Should successfully Modify Rate before starting the STO", async () => {
            let stoId = 0;
            let snapId = await takeSnapshot();
            await I_POLYPeggedSTO_Array[stoId].modifyRate(new BN(1).mul(e16), { from: ISSUER });

            assert.equal((await I_POLYPeggedSTO_Array[stoId].rate.call()).toString(), new BN(1).mul(e16).toString(), "Rate doesn't set as expected");
            await revertToSnapshot(snapId);
        });
        ...
        it("Should Fail to Modify Rate after starting the STO", async () => {
            ...
            await increaseTime(duration.days(3));
            assert.equal(await I_POLYPeggedSTO_Array[stoId].isOpen(), true, "STO is not Open");
            await catchRevert(I_POLYPeggedSTO_Array[stoId].modifyRate(new BN(1).mul(e16), { from: ISSUER }));
            await revertToSnapshot(snapId);
        });
        ...
        it("Should successfully Modify Limits before startTime -- fail because of bad owner", async () => {
            ...
            await catchRevert(I_POLYPeggedSTO_Array[stoId].modifyLimits(new BN(1).mul(e18), new BN(15).mul(e18), new BN(3), { from: POLYMATH }));
        });
        ...
        it("Should successfully Modify Limits before startTime", async () => {
            ...
            await I_POLYPeggedSTO_Array[stoId].modifyLimits(new BN(1).mul(e18), new BN(15).mul(e18), new BN(3), { from: ISSUER });
            assert.equal(
                (await I_POLYPeggedSTO_Array[stoId].minimumInvestment.call()).toString(),
        ...
        it("Should Fail to Modify Limits after startTime", async () => {
            ...
            await increaseTime(duration.days(3));
            assert.equal(await I_POLYPeggedSTO_Array[stoId].isOpen(), true, "STO is not Open");
            await catchRevert(I_POLYPeggedSTO_Array[stoId].modifyLimits(new BN(2).mul(e18), new BN(20).mul(e18), new BN(4), { from: ISSUER }));
            await revertToSnapshot(snapId);
        });
        ...
        it("Should successfully modify the treasury wallet address -- failed because of bad owner", async() => {
            await catchRevert(
                I_POLYPeggedSTO_Array[0].modifyTreasuryWallet(ISSUER, {from: NONACCREDITED1})
            );
        });
        ...
        it("Should successfully modify the treasury wallet of the multiple STOs", async() => {
            let snapId = await takeSnapshot();
            let stoId = 0;
            await I_POLYPeggedSTO_Array[stoId].modifyTreasuryWallet(address_zero, {from: ISSUER});
            assert.equal(
                await I_POLYPeggedSTO_Array[stoId].treasuryWallet.call(),
                address_zero,
            ...
            stoId = 1;
            await I_POLYPeggedSTO_Array[stoId].modifyTreasuryWallet(TREASURYWALLET, {from: ISSUER});
            assert.equal(
                await I_POLYPeggedSTO_Array[stoId].treasuryWallet.call(),
                TREASURYWALLET,
                "Incorrect treasuryWallet"
        ...
        it("Should allow non-matching beneficiary -- failed because of bad owner", async () => {
            ...
            await catchRevert(I_POLYPeggedSTO_Array[0].changeAllowBeneficialInvestments(true, { from: POLYMATH }));
        });

        it("Should allow non-matching beneficiary", async () => {
            await I_POLYPeggedSTO_Array[0].changeAllowBeneficialInvestments(true, { from: ISSUER });
            assert.equal(await I_POLYPeggedSTO_Array[0].allowBeneficialInvestments(), true, "allowBeneficialInvestments should be true");
        });

        it("Should allow non-matching beneficiary -- failed because it is already active", async () => {
            await catchRevert(I_POLYPeggedSTO_Array[0].changeAllowBeneficialInvestments(true, { from: ISSUER }));
            await revertToSnapshot(snapId);
            assert.equal(await I_POLYPeggedSTO_Array[0].allowBeneficialInvestments(), false, "allowBeneficialInvestments should be false");
        });
    });
    ...
    describe("Test buying failure conditions", async () => {
        it("Should fail if before STO start time", async () => {
            ...
            assert.equal(await I_POLYPeggedSTO_Array[stoId].isOpen(), false, "STO is Open");
            ...
            // NONACCREDITED POLY
            await catchRevert(I_POLYPeggedSTO_Array[stoId].buyWithPOLY(NONACCREDITED1, investment_POLY, { from: NONACCREDITED1 }));
            // ACCREDITED POLY
            await catchRevert(I_POLYPeggedSTO_Array[stoId].buyWithPOLY(ACCREDITED1, investment_POLY, { from: ACCREDITED1 }));
            await revertToSnapshot(snapId);
        });
        ...
        it("Should fail if not whitelisted or canNotBuyFromSTO Flag is true", async () => {
            ...
            await I_PolyToken.approve(I_POLYPeggedSTO_Array[stoId].address, investment_POLY, { from: NONACCREDITED2 });
            await I_PolyToken.getTokens(investment_POLY, ACCREDITED1);
            await I_PolyToken.approve(I_POLYPeggedSTO_Array[stoId].address, investment_POLY, { from: ACCREDITED1 });
            await I_PolyToken.getTokens(investment_POLY, ACCREDITED2);
            await I_PolyToken.approve(I_POLYPeggedSTO_Array[stoId].address, investment_POLY, { from: ACCREDITED2 });

            // NONACCREDITED POLY
            await catchRevert(I_POLYPeggedSTO_Array[stoId].buyWithPOLY(NONACCREDITED1, investment_POLY, { from: NONACCREDITED1 }));
            await catchRevert(I_POLYPeggedSTO_Array[stoId].buyWithPOLY(NONACCREDITED2, investment_POLY, { from: NONACCREDITED2 }));

            // ACCREDITED POLY
            await catchRevert(I_POLYPeggedSTO_Array[stoId].buyWithPOLY(ACCREDITED1, investment_POLY, { from: ACCREDITED1 }));
            await catchRevert(I_POLYPeggedSTO_Array[stoId].buyWithPOLY(ACCREDITED2, investment_POLY, { from: ACCREDITED2 }));
        ...
        it("Should successfully pause the STO and make investments fail, then unpause and succeed", async () => {
            ...
            // Pause the STO
            await I_POLYPeggedSTO_Array[stoId].pause({ from: ISSUER });
            assert.equal(await I_POLYPeggedSTO_Array[stoId].paused.call(), true, "STO did not pause successfully");
            ...
            // Unpause the STO
            await I_POLYPeggedSTO_Array[stoId].unpause({ from: ISSUER });
            assert.equal(await I_POLYPeggedSTO_Array[stoId].paused.call(), false, "STO did not unpause successfully");

            await I_POLYPeggedSTO_Array[stoId].buyWithPOLY(NONACCREDITED1, investment_POLY, { from: NONACCREDITED1 });
            await I_POLYPeggedSTO_Array[stoId].buyWithPOLY(ACCREDITED1, investment_POLY, { from: ACCREDITED1 });
        ...
        it("Should fail if finalized", async () => {
            ...
            // Finalize STO
            await I_POLYPeggedSTO_Array[stoId].finalize(false, { from: ISSUER });
            assert.equal(await I_POLYPeggedSTO_Array[stoId].isFinalized.call(), true, "STO has not been finalized");
            assert.equal(await I_POLYPeggedSTO_Array[stoId].isOpen(), false, "STO is Open");

            // Attempt to call function again with minting unsold tokens - should fail as already finalized
            await catchRevert(I_POLYPeggedSTO_Array[stoId].finalize(true, { from: ISSUER }));
        ...
        it("Should fail if STO has insufficinet POLY spend approved by investor", async () => {
            ...
            let investment_POLY = new BN(web3.utils.toWei("10000", "ether")); // Invest 10000 POLY
            await I_PolyToken.getTokens(investment_POLY, NONACCREDITED1);
            await I_PolyToken.approve(I_POLYPeggedSTO_Array[stoId].address, approved_POLY, { from: NONACCREDITED1 });
            await I_PolyToken.getTokens(investment_POLY, ACCREDITED1);
            await I_PolyToken.approve(I_POLYPeggedSTO_Array[stoId].address, approved_POLY, { from: ACCREDITED1 });
        ...
        it("Should fail if Beneficial Investments are not allowed then succeed when they are allowed", async () => {
            ...
            await increaseTime(duration.days(3));
            assert.equal(await I_POLYPeggedSTO_Array[stoId].isOpen(), true, "STO is not Open");
            assert.equal(await I_POLYPeggedSTO_Array[stoId].allowBeneficialInvestments(), false, "allowBeneficialInvestments should be false");
            ...
            // NONACCREDITED POLY
            await catchRevert(I_POLYPeggedSTO_Array[stoId].buyWithPOLY(NONACCREDITED1, investment_POLY, { from: NONACCREDITED2 }));

            // ACCREDITED POLY
            await catchRevert(I_POLYPeggedSTO_Array[stoId].buyWithPOLY(ACCREDITED1, investment_POLY, { from: ACCREDITED2 }));

            // ALLOW BENEFICIAL INVESTMENTS
            await I_POLYPeggedSTO_Array[stoId].changeAllowBeneficialInvestments(true, { from: ISSUER });
            assert.equal(await I_POLYPeggedSTO_Array[stoId].allowBeneficialInvestments(), true, "allowBeneficialInvestments should be true");

            // NONACCREDITED POLY
            await I_POLYPeggedSTO_Array[stoId].buyWithPOLY(NONACCREDITED1, investment_POLY, { from: NONACCREDITED2 });

            // ACCREDITED POLY
            await I_POLYPeggedSTO_Array[stoId].buyWithPOLY(ACCREDITED1, investment_POLY, { from: ACCREDITED2 });
        ...
        it("Should fail if Beneficial Investments are allowed but beneficiary = address_zero", async () => {
            ...
            // Buy tokens
            await catchRevert(I_POLYPeggedSTO_Array[stoId].buyWithPOLY(address_zero, investment_POLY, { from: ACCREDITED1 }));
        ...
        it("Should fail if for non-accredited investors if nonAccreditedLimit has been reached", async () => {
            ...
            let nonAccreditedLimitOverride = new BN(web3.utils.toWei("5000", "ether")); //Limit 5000 USD
            await I_POLYPeggedSTO_Array[stoId].changeNonAccreditedLimit([NONACCREDITED2], [nonAccreditedLimitOverride], { from: ISSUER });
            ...
            await I_PolyToken.approve(I_POLYPeggedSTO_Array[stoId].address, requiredPOLY, { from: NONACCREDITED1 });
            await I_PolyToken.getTokens(requiredPOLY, NONACCREDITED2);
            await I_PolyToken.approve(I_POLYPeggedSTO_Array[stoId].address, requiredPOLY, { from: NONACCREDITED2 });
            await I_PolyToken.getTokens(requiredPOLY, ACCREDITED1);
            await I_PolyToken.approve(I_POLYPeggedSTO_Array[stoId].address, requiredPOLY, { from: ACCREDITED1 });

            // Buy up to Default limit - all should succeed
            await I_POLYPeggedSTO_Array[stoId].buyWithPOLY(NONACCREDITED1, investment_POLY, { from: NONACCREDITED1 });
            await I_POLYPeggedSTO_Array[stoId].buyWithPOLY(NONACCREDITED2, investment_POLY, { from: NONACCREDITED2 });
            await I_POLYPeggedSTO_Array[stoId].buyWithPOLY(ACCREDITED1, investment_POLY, { from: ACCREDITED1 });
        ...
        it("Should fail if for non-accredited investors once maxNonAccreditedInvestors limit is reached", async () => {
            ...
            await I_PolyToken.approve(I_POLYPeggedSTO_Array[stoId].address, investment_POLY, { from: INVESTOR1 });
            await I_PolyToken.getTokens(investment_POLY, INVESTOR2);
            await I_PolyToken.approve(I_POLYPeggedSTO_Array[stoId].address, investment_POLY, { from: INVESTOR2 });
            await I_PolyToken.getTokens(investment_POLY, INVESTOR3);
            await I_PolyToken.approve(I_POLYPeggedSTO_Array[stoId].address, investment_POLY, { from: INVESTOR3 });
            await I_PolyToken.getTokens(investment_POLY, ACCREDITED1);
            await I_PolyToken.approve(I_POLYPeggedSTO_Array[stoId].address, investment_POLY, { from: ACCREDITED1 });
            await I_PolyToken.getTokens(investment_POLY, ACCREDITED2);
            await I_PolyToken.approve(I_POLYPeggedSTO_Array[stoId].address, investment_POLY, { from: ACCREDITED2 });
            ...
            await I_POLYPeggedSTO_Array[stoId].buyWithPOLY(NONACCREDITED2, investment_POLY, { from: NONACCREDITED2 });
            await I_POLYPeggedSTO_Array[stoId].buyWithPOLY(ACCREDITED1, investment_POLY, { from: ACCREDITED1 });
            await I_POLYPeggedSTO_Array[stoId].buyWithPOLY(INVESTOR1, investment_POLY, { from: INVESTOR1 });
            await I_POLYPeggedSTO_Array[stoId].buyWithPOLY(INVESTOR2, investment_POLY, { from: INVESTOR2 });
            // Fifth non-accredited investor should fail due to limit being reached
            await catchRevert(I_POLYPeggedSTO_Array[stoId].buyWithPOLY(INVESTOR3, investment_POLY, { from: INVESTOR3 }));
            // verify accredited investors can still invest
            await I_POLYPeggedSTO_Array[stoId].buyWithPOLY(ACCREDITED2, investment_POLY, { from: ACCREDITED2 });
            await revertToSnapshot(snapId);
        });
        ...
        it("Should fail if cap has been reached", async () => {
            ...
            await increaseTime(duration.days(3));
            assert.equal(await I_POLYPeggedSTO_Array[stoId].isOpen(), true, "STO is not Open");
            assert.equal(await I_POLYPeggedSTO_Array[stoId].capReached(), false, "STO cap already reached");
            ...
            await I_PolyToken.approve(I_POLYPeggedSTO_Array[stoId].address, max_investment, { from: ACCREDITED1 });
            await I_PolyToken.getTokens(investment_POLY, ACCREDITED2);
            await I_PolyToken.approve(I_POLYPeggedSTO_Array[stoId].address, investment_POLY, { from: ACCREDITED2 });
            await I_PolyToken.getTokens(investment_POLY, NONACCREDITED1);
            await I_PolyToken.approve(I_POLYPeggedSTO_Array[stoId].address, investment_POLY, { from: NONACCREDITED1 });
            ...
            await I_POLYPeggedSTO_Array[stoId].buyWithPOLY(ACCREDITED1, max_investment, { from: ACCREDITED1 });
            assert.equal(await I_POLYPeggedSTO_Array[stoId].isOpen(), false, "STO is Open");
            assert.equal(await I_POLYPeggedSTO_Array[stoId].capReached(), true, "STO cap has not been reached");
        ...
        it("Should revert if ETH is sent to the STO Contract", async () => {
            ...
            await catchRevert(web3.eth.sendTransaction({
                from: NONACCREDITED1,
                to: I_POLYPeggedSTO_Array[stoId].address,
    ...
    describe("Prep STO", async () => {
        ...
        it("Should successfully get investor accredited status for the STOs", async () => {
             let stoId = 0;
             let totalStatus = await I_POLYPeggedSTO_Array[stoId].getAccreditedData.call();
             console.log(totalStatus);
             assert.equal(totalStatus[0][0], NONACCREDITED1, "Account match1");
             assert.equal(totalStatus[0][1], ACCREDITED1, "Account match2");
             assert.equal(totalStatus[1][0], false, "Account match3");
             assert.equal(totalStatus[1][1], true, "Account match4");
             assert.equal(totalStatus[2][0].toNumber(), 0, "override match5");
             assert.equal(totalStatus[2][1].toNumber(), 0, "override match6");
         });
    });
    ...
    describe("Buy Tokens", async () => {
        it("Should successfully buy using buyWithPOLY for NONACCREDITED1", async () => {
            ...
            let init_STOTokenSold = await I_POLYPeggedSTO_Array[stoId].getTokensSold();
            let init_STOETHBal = new BN(await web3.eth.getBalance(I_POLYPeggedSTO_Array[stoId].address));
            let init_STOPOLYBal = await I_PolyToken.balanceOf(I_POLYPeggedSTO_Array[stoId].address);
            let init_RaisedETH = await I_POLYPeggedSTO_Array[stoId].fundsRaised.call(ETH);
            let init_RaisedPOLY = await I_POLYPeggedSTO_Array[stoId].fundsRaised.call(POLY);
            let init_RaisedSC = await I_POLYPeggedSTO_Array[stoId].fundsRaised.call(SC);
            ...
            let init_InvestorCount = await I_POLYPeggedSTO_Array[stoId].investorCount.call();
            let init_nonAccreditedCount = await I_POLYPeggedSTO_Array[stoId].nonAccreditedCount.call();
            let init_investorInvestedPOLY = await I_POLYPeggedSTO_Array[stoId].investorInvestedPOLY(NONACCREDITED1);
            let init_investorInvestedUSD = await I_POLYPeggedSTO_Array[stoId].investorInvestedUSD(NONACCREDITED1);
            ...
            let final_STOTokenSold = await I_POLYPeggedSTO_Array[stoId].getTokensSold();
            let final_STOETHBal = new BN(await web3.eth.getBalance(I_POLYPeggedSTO_Array[stoId].address));
            let final_STOPOLYBal = await I_PolyToken.balanceOf(I_POLYPeggedSTO_Array[stoId].address);
            let final_RaisedETH = await I_POLYPeggedSTO_Array[stoId].fundsRaised.call(ETH);
            let final_RaisedPOLY = await I_POLYPeggedSTO_Array[stoId].fundsRaised.call(POLY);
            let final_RaisedSC = await I_POLYPeggedSTO_Array[stoId].fundsRaised.call(SC);
            ...
            let final_InvestorCount = await I_POLYPeggedSTO_Array[stoId].investorCount.call();
            let final_nonAccreditedCount = await I_POLYPeggedSTO_Array[stoId].nonAccreditedCount.call();
            let final_investorInvestedPOLY = await I_POLYPeggedSTO_Array[stoId].investorInvestedPOLY(NONACCREDITED1);
            let final_investorInvestedUSD = await I_POLYPeggedSTO_Array[stoId].investorInvestedUSD(NONACCREDITED1);
            ...
            assert.equal(
                final_investorInvestedPOLY.toString(),
        ...
        it("Should successfully buy using buyWithPOLY for ACCREDITED1", async () => {
            ...
            let init_InvestorCount = await I_POLYPeggedSTO_Array[stoId].investorCount.call();
            let init_nonAccreditedCount = await I_POLYPeggedSTO_Array[stoId].nonAccreditedCount.call();
            let init_investorInvestedPOLY = await I_POLYPeggedSTO_Array[stoId].investorInvestedPOLY(ACCREDITED1);
            let init_investorInvestedUSD = await I_POLYPeggedSTO_Array[stoId].investorInvestedUSD(ACCREDITED1);
            ...
            // Buy With POLY
            let tx2 = await I_POLYPeggedSTO_Array[stoId].buyWithPOLY(ACCREDITED1, investment_POLY, {
                from: ACCREDITED1,
                gasPrice: GAS_PRICE
            });
            ...
            let final_InvestorCount = await I_POLYPeggedSTO_Array[stoId].investorCount.call();
            let final_nonAccreditedCount = await I_POLYPeggedSTO_Array[stoId].nonAccreditedCount.call();
            let final_investorInvestedPOLY = await I_POLYPeggedSTO_Array[stoId].investorInvestedPOLY(ACCREDITED1);
            let final_investorInvestedUSD = await I_POLYPeggedSTO_Array[stoId].investorInvestedUSD(ACCREDITED1);
        ...
        it("Should successfully modify NONACCREDITED cap for NONACCREDITED1 and NONACCREDITED2 -- failed array length mismatch", async () => {
            ...
            await catchRevert(I_POLYPeggedSTO_Array[stoId].changeNonAccreditedLimit([NONACCREDITED1, NONACCREDITED2], [_nonAccreditedLimit[stoId].mul(new BN(2))], {
                from: ISSUER
            }));
        });
        ...
        it("Should successfully modify NONACCREDITED cap for NONACCREDITED1", async () => {
            let stoId = 0;
            console.log("          Current investment: ".grey + (await I_POLYPeggedSTO_Array[stoId].investorInvestedPOLY.call(NONACCREDITED1)).toString().grey);

            await I_POLYPeggedSTO_Array[stoId].changeNonAccreditedLimit([NONACCREDITED1], [_nonAccreditedLimit[stoId].mul(new BN(2))], {
                from: ISSUER
            });
            let investorLimit = await I_POLYPeggedSTO_Array[stoId].nonAccreditedLimitOverride.call(NONACCREDITED1);
        ...
        it("Should successfully buy a partial amount and refund balance when reaching NONACCREDITED limit", async () => {
            ...
            let investorLimit = await I_POLYPeggedSTO_Array[stoId].nonAccreditedLimitOverride.call(NONACCREDITED1);
            let investedUSD = await I_POLYPeggedSTO_Array[stoId].investorInvestedUSD.call(NONACCREDITED1);
            let rate = await I_POLYPeggedSTO_Array[stoId].rate.call();
            let surplus_POLY = new BN (web3.utils.toWei("1234", "ether")); // Amount above the limit to invest
            let surplus_USD = new BN (surplus_POLY.mul(await I_POLYOracle.getPrice.call()).div(e18));

            let investment_USD = investorLimit.sub(investedUSD).add(surplus_USD); // Calculate investment amount in USD
            ...
            assert.notEqual(init_investorInvestedPOLY, 0, "Investor has not already invested");
            ...
            assert.equal(
                final_investorInvestedPOLY.toString(),
                init_investorInvestedPOLY.add(investment_POLY.sub(surplus_POLY)).toString(),
        ...
        it("Should successfully buy a granular amount and refund balance when buying a indivisible token with POLY", async () => {
            ...
            let surplus_Tokens = surplus_USD.mul(rate).div(e18);
            let expectedTokens = (investment_USD.mul(rate).div(e18)).sub(surplus_Tokens);
            ...
            console.log("          Old total investment: ".grey + init_investorInvestedPOLY.toString().grey + " POLY".grey);

            assert.notEqual(init_investorInvestedPOLY, 0, "Investor has not already invested");
            assert.notEqual(init_investorInvestedUSD, 0, "Investor has not already invested");

            // Buy With POLY
            let tx1 = await I_POLYPeggedSTO_Array[stoId].buyWithPOLY(ACCREDITED1, investment_POLY, {
                from: ACCREDITED1,
                gasPrice: GAS_PRICE
            });
            let gasCost2 = new BN(GAS_PRICE).mul(new BN(tx1.receipt.gasUsed));
        ...
        it("Should fail due to calculated number of tokens being less than the granularity when buying a indivisible token", async () => {
            ...
            let investorInvestedPOLY = await I_POLYPeggedSTO_Array[stoId].investorInvestedPOLY(ACCREDITED1);
            let minimumInvestment = await I_POLYPeggedSTO_Array[stoId].minimumInvestment.call();
            let minInvested = investorInvestedPOLY.gte(minimumInvestment);
            ...
            // Buy With POLY
            await catchRevert(I_POLYPeggedSTO_Array[stoId].buyWithPOLY(ACCREDITED1, investment_POLY, {
                from: ACCREDITED1,
                gasPrice: GAS_PRICE
            }));
            await I_SecurityToken.changeGranularity(1, { from: ISSUER });
        ...
        it("Should fail to buy if granularity prevents buying remaining tokens when buying up to the cap", async () => {
            ...
            let remainingUSD = remainingTokens.mul(e18).div(rate); // Calculate investment amount
            let remainingPOLY = remainingUSD.mul(e18).div(await I_POLYOracle.getPrice.call());
            ...
            await I_PolyToken.approve(I_POLYPeggedSTO_Array[stoId].address, investment_POLY1.add(investment_POLY2), { from: ACCREDITED1 });

            // Buy first batch With POLY
            let tx1 = await I_POLYPeggedSTO_Array[stoId].buyWithPOLY(ACCREDITED1, investment_POLY1, {
                from: ACCREDITED1,
                gasPrice: GAS_PRICE
            });
            let gasCost2 = new BN(GAS_PRICE).mul(new BN(tx1.receipt.gasUsed));
            ...
            let new_STOTokenSold = await I_POLYPeggedSTO_Array[stoId].getTokensSold();
            ...
            await catchRevert(I_POLYPeggedSTO_Array[stoId].buyWithPOLY(ACCREDITED1, investment_POLY2, {
                from: ACCREDITED1,
                gasPrice: GAS_PRICE
            }));
        ...
        it("Should successfully buy a partial amount and refund balance when reaching the cap", async () => {
            ...
            let investment_USD = (remainingTokens.mul(e18).div(rate)).add(surplus_USD); // Calculate investment amount in USD
            let investment_POLY = investment_USD.mul(e18).div(await I_POLYOracle.getPrice.call()); // Calculate investment amount in POLY
    ...
    describe("Test finalizing the STO", async () => {
        it("Should successfully finalize without minting unsold tokens to the treasury wallet", async () => {
            ...
            let final_InvestorCount = await I_POLYPeggedSTO_Array[stoId].investorCount.call();
            let final_nonAccreditedCount = await I_POLYPeggedSTO_Array[stoId].nonAccreditedCount.call();
            let final_TokensReturned = await I_POLYPeggedSTO_Array[stoId].finalAmountReturned.call();
        ...
        it("Should successfully finalize and mint unsold tokens to the treasury wallet -- fail local treasury wallet not whitelisted", async () => {
            ...
            assert.equal(await I_POLYPeggedSTO_Array[stoId].isFinalized.call(), false, "STO has been finalized");
            assert.equal(await I_POLYPeggedSTO_Array[stoId].isOpen(), true, "STO not Open");
        });
        ...
        it("Should successfully finalize and mint unsold tokens to the treasury wallet -- fail because of bad owner", async () => {
            ...
            // Finalize STO without minting
            await catchRevert(I_POLYPeggedSTO_Array[stoId].finalize(true, { from: POLYMATH }));
        ...
        it("Should successfully finalize and mint unsold tokens to the local treasury wallet", async () => {
            ...
            // Finalize STO without minting
            await I_POLYPeggedSTO_Array[stoId].finalize(true, { from: ISSUER });
            assert.equal(await I_POLYPeggedSTO_Array[stoId].isFinalized.call(), true, "STO has not been finalized");
            assert.equal(await I_POLYPeggedSTO_Array[stoId].isOpen(), false, "STO is Open");
    ...
    describe("Test getter functions", async () => {
        it("Should get the right number of investors", async () => {
            assert.equal(
                (await I_POLYPeggedSTO_Array[0].investorCount.call()).toString(),
                (await I_POLYPeggedSTO_Array[0].investorCount()).toString(),
                "Investor count not changed as expected"
            );
            assert.equal(
                (await I_POLYPeggedSTO_Array[1].investorCount.call()).toString(),
                (await I_POLYPeggedSTO_Array[1].investorCount()).toString(),
                "Investor count not changed as expected"
            );
        });
        ...
        it("Should get the right amounts invested", async () => {
            assert.equal(
                (await I_POLYPeggedSTO_Array[0].fundsRaised.call(ETH)).toString(),
                (await I_POLYPeggedSTO_Array[0].getRaised(0)).toString(),
                "getRaisedEther not changed as expected"
            );
            assert.equal(
                (await I_POLYPeggedSTO_Array[0].fundsRaised.call(POLY)).toString(),
                (await I_POLYPeggedSTO_Array[0].getRaised(1)).toString(),
                "getRaisedPOLY not changed as expected"
            );
            assert.equal(
                (await I_POLYPeggedSTO_Array[0].fundsRaisedUSD.call()).toString(),
                (await I_POLYPeggedSTO_Array[0].fundsRaisedUSD()).toString(),
        ...
        it("Should return correct STO details", async () => {
            ...
            */
            // Get STO Details
            let stoDetails = await I_POLYPeggedSTO_Array[stoId].getSTODetails.call();
            ...
            let totalTokensSold = await I_POLYPeggedSTO_Array[stoId].totalTokensSold.call();
            let fundsRaisedPOLY = await I_POLYPeggedSTO_Array[stoId].fundsRaised.call(POLY);
            let fundsRaisedUSD = await I_POLYPeggedSTO_Array[stoId].fundsRaisedUSD.call();
            let investorCount = await I_POLYPeggedSTO_Array[stoId].investorCount.call();
            let nonAccreditedCount = await I_POLYPeggedSTO_Array[stoId].nonAccreditedCount.call();
        ...
        it("Should get the correct POLY token address", async () => {
            assert.equal(
                ...
                (await I_POLYPeggedSTO_Array[0].polyToken.call()).toString(),
                "POLY token addresses don't match"
            );
            assert.equal(
                (await I_PolyToken.address).toString(),
                (await I_POLYPeggedSTO_Array[1].polyToken.call()).toString(),
                "POLY token addresses don't match"
            );
        });
        ...
        it("Should get the correct security token address", async () => {
            assert.equal(
                (await I_POLYPeggedSTO_Array[0].securityToken.call()).toString(),
                (await I_SecurityToken.address).toString(),
                "Security token addresses don't match"
            );
            assert.equal(
                (await I_POLYPeggedSTO_Array[1].securityToken.call()).toString(),
                (await I_SecurityToken.address).toString(),
                "Security token addresses don't match"
            );
        });
        ...
        it("Should get the correct STO factory address", async () => {
            assert.equal(
                (await I_POLYPeggedSTO_Array[0].factory.call()).toString(),
                (await I_POLYPeggedSTOFactory.address).toString(),
                "Factory addresses don't match"
            );
            assert.equal(
                (await I_POLYPeggedSTO_Array[1].factory.call()).toString(),
                (await I_POLYPeggedSTOFactory.address).toString(),
    ...
    describe("Miscelanious tests", async () => {
        it("Should pause before end time and fail to pause after end time", async () => {
            ...
            await catchRevert(I_POLYPeggedSTO_Array[stoId].pause({ from: ISSUER }));
            assert.equal(await I_POLYPeggedSTO_Array[stoId].paused.call(), false, "STO paused");

            await revertToSnapshot(snapId);
        });

        it("Should fail to deploy a new POLYPeggedSTOFactory -- logic contract is address zero", async () => {
        await catchRevert(POLYPeggedSTOFactory.new(STOSetupCost, new BN(0), address_zero, I_PolymathRegistry.address, { from: POLYMATH }));
        });

        it("Should fail to deploy a new POLYPeggedSTOProxy -- logic contract is address zero", async () => {
        await catchRevert(POLYPeggedSTOProxy.new("3.0.0", I_SecurityToken.address, I_PolyToken.address, address_zero, { from: POLYMATH }));
        });
    });
    ...
    // FACTORY TESTS //
    ///////////////////
    describe("Test cases for the POLYPeggedSTOFactory", async () => {
        it("Should get the exact details of the factory", async () => {
            assert.equal((await I_POLYPeggedSTOFactory.setupCost.call()).toString(), STOSetupCost);
            assert.equal((await I_POLYPeggedSTOFactory.setupCostInPoly.call()).toString(), STOSetupCost);
            assert.equal((await I_POLYPeggedSTOFactory.types.call())[0], 3);
            assert.equal(web3.utils.hexToString(await I_POLYPeggedSTOFactory.name.call()), "POLYPeggedSTO", "Wrong Module added");
            assert.equal(
                await I_POLYPeggedSTOFactory.description.call(),
            ...
            assert.equal(await I_POLYPeggedSTOFactory.title.call(), "POLY - Pegged to USD - STO", "Wrong Module added");
            assert.equal(await I_POLYPeggedSTOFactory.version.call(), "3.0.0");
            let tags = await I_POLYPeggedSTOFactory.tags.call();
            assert.equal(web3.utils.hexToString(tags[0]), "Capped");
            assert.equal(web3.utils.hexToString(tags[1]), "Pegged");
        ...
        it("Should fail to change the setup cost for STO Factory - bad owner", async () => {
            ...
            await catchRevert(I_POLYPeggedSTOFactory.changeSetupCost(newSetupCost, { from: ISSUER }));
        });
        ...
        it("Should successfully change the setup cost for STO Factory", async () => {
            ...
            await I_POLYPeggedSTOFactory.changeSetupCost(newSetupCost, { from: POLYMATH });
            assert.equal((await I_POLYPeggedSTOFactory.setupCost.call()).toString(), newSetupCost.toString(), "Setup Cost mismatch");
            assert.equal((await I_POLYPeggedSTOFactory.setupCostInPoly.call()).toString(), newSetupCostPOLY.toString(), "Setup Cost POLY mismatch");
        });

        it("Should fail to change the title -- bad owner", async () => {
            await catchRevert(I_POLYPeggedSTOFactory.changeTitle("STO Pegged POLY", { from: ISSUER }));
        });

        it("Should fail to change the title -- zero length", async () => {
            await catchRevert(I_POLYPeggedSTOFactory.changeTitle("", { from: POLYMATH }));
        });

        it("Should successfully change the title", async () => {
            await I_POLYPeggedSTOFactory.changeTitle("STO Pegged POLY", { from: POLYMATH });
            assert.equal(await I_POLYPeggedSTOFactory.title.call(), "STO Pegged POLY", "Title doesn't get changed");
        });

        it("Should fail to change the description -- bad owner", async () => {
            await catchRevert(I_POLYPeggedSTOFactory.changeDescription("It is only a STO", { from: ISSUER }));
        });

        it("Should fail to change the description -- zero length", async () => {
            await catchRevert(I_POLYPeggedSTOFactory.changeDescription("", { from: POLYMATH }));
        });

        it("Should successfully change the description", async () => {
            await I_POLYPeggedSTOFactory.changeDescription("It is only a STO", { from: POLYMATH });
            assert.equal(await I_POLYPeggedSTOFactory.description.call(), "It is only a STO", "Description doesn't get changed");
        });

        it("Should fail to change the name -- bad owner", async () => {
            await catchRevert(I_POLYPeggedSTOFactory.changeName(web3.utils.stringToHex("STOPegged"), { from: ISSUER }));
        });

        it("Should fail to change the name -- zero length", async () => {
            await catchRevert(I_POLYPeggedSTOFactory.changeName(web3.utils.stringToHex(""), { from: POLYMATH }));
        });

        it("Should successfully change the name", async () => {
            await I_POLYPeggedSTOFactory.changeName(web3.utils.stringToHex("STOPegged"), { from: POLYMATH });
            assert.equal(web3.utils.hexToString(await I_POLYPeggedSTOFactory.name.call()), "STOPegged", "Name doesn't get changed");
        });

        it("Should fail to change lower and upper Security Token Version Bounds -- bad owner", async () => {
            await catchRevert(I_POLYPeggedSTOFactory.changeSTVersionBounds("lowerBound", [1,1,1], { from: ISSUER }));
            await catchRevert(I_POLYPeggedSTOFactory.changeSTVersionBounds("upperBound", [9,9,9], { from: ISSUER }));
        });

        it("Should successfully change lower and upper Security Token Version Bounds", async () => {
            await I_POLYPeggedSTOFactory.changeSTVersionBounds("lowerBound", [1,2,3], { from: POLYMATH });
            await I_POLYPeggedSTOFactory.changeSTVersionBounds("upperBound", [4,5,6], { from: POLYMATH });
            let lower = await I_POLYPeggedSTOFactory.lowerSTVersionBounds.call();
        ...
        it("Should fail to change tags -- bad owner", async () => {
            ...
            await catchRevert(I_POLYPeggedSTOFactory.changeTags(newTags, { from: ISSUER }));
        });

        it("Should fail to change tags -- tags length = 0", async () => {
            let newTags = [];
            await catchRevert(I_POLYPeggedSTOFactory.changeTags(newTags, { from: POLYMATH }));
        });
        ...
        it("Should successfully change tags", async () => {
            ...
            newTags[1] = web3.utils.stringToHex("Tag2");
            newTags[2] = web3.utils.stringToHex("Tag3");
            await I_POLYPeggedSTOFactory.changeTags(newTags, { from: POLYMATH });
            let tags = await I_POLYPeggedSTOFactory.tags.call();
        ...
        it("Should fail to transfer Ownership -- bad owner", async () => {
            await catchRevert(I_POLYPeggedSTOFactory.transferOwnership(ISSUER, { from: ISSUER }));
        });

        it("Should fail to transfer Ownership -- address zero", async () => {
            await catchRevert(I_POLYPeggedSTOFactory.transferOwnership(address_zero, { from: POLYMATH }));
        });

        it("Should successfully transfer Ownership", async () => {
            await I_POLYPeggedSTOFactory.transferOwnership(ISSUER, { from: POLYMATH });
            assert.equal(await I_POLYPeggedSTOFactory.owner.call(), ISSUER, "Owner doesn't get changed");
        });
    });
});
